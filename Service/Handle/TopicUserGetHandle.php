...
class TopicUserGetHandle extends AbstractTopicHandle
{
    public function handle(RecvBuffer $buffer): bool
    {
        //正则表达式 从  simps-mqtt/user/get/{device_id}  拿到 device_id
        $is_match = preg_match('/simps-mqtt\/user\/get\/(\d+)/', $buffer->getTopic(), $matches);
        if ($is_match) {
            $device_id = $matches[1] ?? '';
